...
class PolymorphicBaseModel(BaseModel):
    ...
    def __init_subclass__(cls, type_name: str = "", is_polymorphic_base: bool = False, **kwargs: ConfigDict):
    ...
    def _reify_type(cls, v: Any, handler: ValidatorFunctionWrapHandler):
        ...
        if t not in cls._registry:
            ...
            raise ValueError(f"Type {t} is not in the known types for {cls!r} ({cls._registry.keys()})")
...
class AtomicLevel(BaseModel, IterateQuantitiesMixin):
    energy: AstropyQty
    g: int
    stage: int
    ...
    def _validate_JLS(self):
        ...
        if any(conds) != all(conds):
            ...
            raise ValueError(f"If one of J, L, S are defined, all must be. Missing ({missing_keys}).")
        return self
        
    def simplify(self, allowed_extensions=None) -> 'SimplifiedAtomicLevel':
...
class LineBroadening(PolymorphicBaseModel, IterateQuantitiesMixin, is_polymorphic_base=True):
    type: str
...
class StarkLinearSutton(LineBroadening, type_name="Stark_Linear_Sutton"):
    ...
    def _validate(self):
        ...
        if any(conds) != all(conds):
            raise ValueError("Must provide both n_upper and n_lower if providing one or the other")
...
class StarkQuadratic(LineBroadening, type_name="Stark_Quadratic"):
    ...
    def _validate_C_4(cls, value: u.Quantity):
        if value.unit.physical_type != 'volumetric flow rate':
            raise ValueError(f"C_4 is expected to be convertible to volumetric flow rate. Got units of {value.unit}")
        return value
...
class WavelengthGrid(PolymorphicBaseModel, IterateQuantitiesMixin, is_polymorphic_base=True):
    type: str
...
class LinearGrid(WavelengthGrid, type_name="Linear"):
    ...
    def simplify(self):
        ...
        return LinearGrid(type=self.type, n_lambda=self.n_lambda, delta_lambda=delta_lambda)
...
class TabulatedGrid(WavelengthGrid, type_name="Tabulated"):
    ...
    def _validate_wavelengths(cls, value: u.Quantity):
        ...
        if len(value.shape) == 0 or len(value.shape) > 1:
            raise ValueError("Wavelengths should be a 1D array of at least one point (relative to line centre).")
...
class AtomicBoundBoundImpl(PolymorphicBaseModel, IterateQuantitiesMixin, is_polymorphic_base=True):
    type: str
    transition: List[str]
...
class VoigtBoundBound(AtomicBoundBound, type_name="Voigt"):
    ...
    def simplify(self):
        broadening = [b.simplify() for b in self.broadening]
        wavelength_grid = self.wavelength_grid.simplify()
        # Aji = 
        # TODO(cmo): Need access to rest of model. Also allowable extensions.
        return self
...
class AtomicBoundFreeImpl(PolymorphicBaseModel, IterateQuantitiesMixin, is_polymorphic_base=True):
    type: str
    transition: List[str]
...
class CollisionalRateImpl(PolymorphicBaseModel, IterateQuantitiesMixin, is_polymorphic_base=True):
    type: str
    temperature: AstropyQty
    data: AstropyQty

    # TODO(cmo): Validate lengths?

CollisionalRate = SerializeAsAny[CollisionalRateImpl]

class OmegaRate(CollisionalRate, type_name="Omega"):
    type: Literal["Omega"]
    ...
    def _validate(self):
        ...
        def test_transition_present(name: str, type_name: str, index: int):
            if name not in self.levels:
                raise ValueError(f"Transition \"{name}\" not found on model (seen on {type_name}[{index}]).")
